...
export default function BookingCartPage() {
  const { items, totals, removeJourney, clearCart } = useCart();
  const router = useRouter();
  const { items: wishlistItems } = useWishlist();

  const journeySlugToWishlistExps = useMemo(() => {
  ...
  return (
    <main className="min-h-screen bg-white">
      <Section background="secondary" padding="xl">
        <Container size="xl">
          <Heading level={1} className="mb-6">Your Booking</Heading>
          {items.length === 0 ? (
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <Button variant="ghost" onClick={() => removeJourney(j.slug)}>Remove</Button>
                        </div>
                        ...
                        <div className="mt-4">
                          ...
                          {j.experiences.length === 0 && !(journeySlugToWishlistExps[j.slug]?.length) ? (
                            <Text size="sm" className="text-gray-500">No extra experiences added</Text>
                          ) : (
                            <ul className="space-y-1">
                              {[...j.experiences, ...(journeySlugToWishlistExps[j.slug] || [])].map((e) => (
                                <li key={e.id} className="flex justify-between text-sm">
                                  <span className="truncate">{e.title}</span>
                                  <span>¥{e.unitPrice}{'pax' in e && (e as any).pax ? ` × ${(e as any).pax}` : ''}</span>
                                </li>
                              ))}
                            </ul>
                          )}
